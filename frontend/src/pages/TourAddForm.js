...
const TourAddForm = () => {
    ...
    useEffect(() => {
        if (param.id) {
            fetch(`https://travel-backend-liart.vercel.app/api/v1/tours/${param.id}`)
                .then(res => res.json())
                .then(res => {
                    setTour(res.data);
                })
    ...
    const handleSubmit = () => {
        const url = param.id
            ? `https://travel-backend-liart.vercel.app/api/v1/tours/${param.id}`
            : "https://travel-backend-liart.vercel.app/api/v1/tours";
